...
export class AuthService {
  static async loginUsuario(email: string, senha: string): Promise<{ token: string; usuario: Usuario } | null> {
    try {
      // Busca usuário com dados do tipo pelo email usando o model
      const usuarioComTipo: any = await UsuarioModel.buscarComTipoUsuario(email);
      if (!usuarioComTipo) {
      ...
      const senhaValida = await bcrypt.compare(senha, usuarioComTipo.senha_usuario);
      if (!senhaValida) {
        logError('Senha inválida', 'service', { email });
        return null;
      }

      // Gera JWT com nome do tipo
      const token = jwt.sign(
        {
          usuario_id: usuarioComTipo.usuario_id,
          tipo_usuario_id: usuarioComTipo.nome_tipo, // Agora usa o nome do tipo
          nome_usuario: usuarioComTipo.nome_usuario,
        },
        process.env.JWT_SECRET as string,
        { expiresIn: '8h' }
      );

      logSuccess('Login realizado com sucesso', 'service', { usuario_id: usuarioComTipo.usuario_id });
      return { token, usuario: usuarioComTipo };
    } catch (error) {
